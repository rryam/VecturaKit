...
final class VecturaMLXKitTests: XCTestCase {
    ...
    private func createVecturaMLXKit(config: VecturaConfig) async throws -> VecturaMLXKit {
        ...
        do {
            ...
            let reason = """
            Failed to initialize VecturaMLXKit: \(error.localizedDescription).
            This may be due to Metal/GPU unavailability, missing Metal Toolchain,
            or MLX device library issues in the test environment.
            """
            throw XCTSkip(
                reason
    ...
    override func setUpWithError() throws {
        ...
        testDirectory = temp.appendingPathComponent(
            "VecturaMLXKitTests-\(uniqueId)",
            isDirectory: true
        )
    ...
    func testAddAndSearch() async throws {
        ...
        XCTAssertEqual(
            results.count,
            1,
            "The search should return one result after adding one document."
        )
        XCTAssertEqual(
            results.first?.text,
            text,
            "The text of the returned document should match the added text."
        )
    }
    ...
    func testDeleteDocuments() async throws {
        ...
        XCTAssertTrue(
            results.isEmpty,
            "After deletion, the document should not be returned in search results."
        )
    }
    ...
    func testUpdateDocument() async throws {
        ...
        XCTAssertEqual(
            results.count,
            1,
            "One document should be returned after update."
        )
        XCTAssertEqual(
            results.first?.text,
            updatedText,
            "The document text should be updated in the search results."
        )
    }
    ...
    func testReset() async throws {
        ...
        XCTAssertTrue(
            results.isEmpty,
            "After a reset, search should return no results."
        )
    }
    ...
    func testSearchMultipleDocuments() async throws {
        ...
        XCTAssertGreaterThanOrEqual(
            results.count,
            2,
            "Should return at least two documents related to 'brown fox'."
        )
    ...
    func testSearchNumResultsLimiting() async throws {
        ...
        XCTAssertEqual(
            results.count,
            3,
            "Should limit the search results to exactly 3 documents."
        )
    }
    ...
    func testSearchNoMatches() async throws {
        ...
        XCTAssertTrue(
            results.isEmpty,
            "Search should return no results when the query does not match any document."
        )
    }
}
