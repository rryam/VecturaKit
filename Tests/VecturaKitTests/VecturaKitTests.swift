...
final class VecturaKitTests: XCTestCase {
    ...
    func testFolderURLModelSource() async throws {
        ...
        /// Proceed as usual now, but loading the Model2Vec model directly from the
        /// local directory instead of downloading it.
    ...
    func testCustomStorageDirectory() async throws {
        ...
        let instance = try await VecturaKit(
            config: .init(name: "test", directoryURL: customDirectoryURL)
        )
        let text = "Test document"
        let id = UUID()
