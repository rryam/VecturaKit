...
public actor MLXEmbedder: VecturaEmbedder {
  ...
  public func embed(texts: [String]) async throws -> [[Float]] {
    await modelContainer.perform { (model: EmbeddingModel, tokenizer, pooling) -> [[Float]] in
      ...
      // Convert MLXArray results to Float arrays with memory-efficient approach.
      // Using autoreleasepool ensures intermediate MLXArray objects are released
      // promptly during conversion, reducing peak memory usage significantly.
      var output: [[Float]] = []
      ...
      // Synchronize GPU and clear cache to release MLX memory
      GPU.synchronize()
      GPU.clearCache()
